...
% remove the infeasible individuals 
idx = 0;
for k = 1 : M
    if population.Chromosomes(k).fitness ~= -inf
        idx = idx + 1;
        tempPopulation.Chromosomes(idx).Gene = population.Chromosomes(k).Gene;
        tempPopulation.Chromosomes(idx).fitness = population.Chromosomes(k).fitness;
    end
end

% replace the current population with the temp
